...
- `internal/keychain` — the only token source. `CLAWCTL_TOKEN_CMD` is honoured first on every platform; otherwise it shells to the OS credential store: `security find-generic-password -w` on macOS (`keychain_darwin.go`), `secret-tool`/`pass` on Linux (`keychain_linux.go`). **Never** falls back to a `CLAWCTL_TOKEN` env var or disk (design principle #2).
...
- `internal/cache` — 60s TTL file cache for `/v1/models` at `$CLAWCTL_CACHE_DIR/models.json`. The *cache layer* fails *open*: if the cache is unreachable or a refresh fails while a stale copy exists, it returns the stale body with a stderr warning so the binary never blocks transport on metadata. (Note: this is distinct from MCP server startup, where an empty agent slug is a *hard* error in `internal/mcpserver` — "fails open" refers to the cache, not slug validation.)
...
- **Cross-platform token store, shell-out not CGO.** `internal/keychain` shells to `security` on macOS and `secret-tool`/`pass` on Linux (build-tagged files), with `CLAWCTL_TOKEN_CMD` as a universal override. Windows (`cmdkey`) is still a future port. Don't link a CGO keystore library; add a build-tagged shell-out file instead.
