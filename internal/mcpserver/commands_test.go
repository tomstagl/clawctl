...
// TestCommandServer_Msg_TaskIDPassthrough verifies the optional task_id
// argument is echoed back on the response envelope (A2A taskId alignment).
func TestCommandServer_Msg_TaskIDPassthrough(t *testing.T) {
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
	...
	res, err := cs.CallTool(context.Background(), &mcp.CallToolParams{
		Name:      "clawctl_msg",
		Arguments: map[string]any{"agent": "concierge", "text": "hi", "task_id": "task-42"},
	})
	if err != nil {
		t.Fatalf("CallTool: %v", err)
	}
	...
	if got := env["task_id"]; got != "task-42" {
		t.Errorf("task_id = %v, want task-42", got)
	}
}
